...
en français (car l'anglish c'est compliqué): le graphique nous représente le temps de jeu vidéo effecté par les différents individus de notre échantillonage on peut constater que les plus grands temps de jeu vidéo sont réalisé par les jeunes individus. Les personnes plus agés ont un temps de jeu en général moins élevé.

The chart represent the time spent on video games for the different subjects of our sample, and we can see that people that play the most are the youngers ones. The old subjects play less often in general
...
```
la tranche d'age 20 - 40 est la plus imprtante mais c'est du au valeur extrème qui modifie la moyenne, l'autre valeur étrange est la valeur des plus 80 là c'est du un petit nombre d'individus
...
Sur ce graphique on peut remarqué que les garçons ont un temps de jeu plus important que j'ai les filles.. L'autre constatation est que le temps de jeu des garçons est très varié par contre chez les filles le temps de jeu est le même chez beaucoup de filles.

On this chart, we can see that the boys overall plays more than girls. We can also see that the playing time for the boys is very varied, however, the overal video game playing time for girls is approximately  the same and not very varied 
...
La répartition est plus varié que pour le temps de jeu mais la partie la plus jeune de la population passe un temps plus important que les autres parties de la population .
...
Plus on grandit moins on regarde les réseaux sociaux, cependant il y a une moyenne pour la tranche d'age 20-40 car c'est à cette période que les réseaux sociaux ont vu le jour.
